...
// Limits
   ...
   shared_memory->setpointPID.sp = 0 ;
...
// NOTE: This function has been editted from it's original so only 1 wheel turns
//       Search the text for "NOTE: Here" to find changes
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
// how forward command should be carried out
   ...
   if (direction == SCW) {
      //NOTE: Here
      updateMotor(M2, CCW, HARD, distance*2, 2*velocity) ;
      updateMotor(M1, CW, HARD, distance, 0) ;
   } else if (direction == SCCW) {
      //NOTE: Here
      updateMotor(M1, CCW, HARD, distance*2, 2*velocity) ;
      updateMotor(M2, CW, HARD, distance, 0) ;
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//Due to restructuring, this function has been moved to navigation.c
//void testRobot(void) {
