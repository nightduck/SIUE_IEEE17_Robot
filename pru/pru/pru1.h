// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//
// Some new variables and define for the NeoPixel array
// Added by gle:26Nov2016
// NEO_CLK is GPIO0[26]
// Some macros added to end of file
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#define     bitCnt      r27.b0
#define     byteCnt     r27.b1
#define     data        r27.b2

#define     MAP_OS      48

// Currently not using the delay value
// Change it to the mode variable
// mode = 1 means neopixel array
...
#define         PRU_EVTOUT_1              4          // event number for PRU 1 to ARM interrupt
...
#define     gpio0               r12
#define     read_gpio0          r15
#define     set_gpio0           r16
#define     clr_gpio0           r17
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// read_mode_value and store_mode_value
//
// Description:
//	read in the delay loop value from sharedMemory (written by host)
// 
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.macro		read_mode_value
			lbbo	mode, sharedMem, MODE_OS, 4	
.endm
...
.macro 		store_state
			sbbo	stateReg, sharedMem, STATE_OS, 4
.endm
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.macro		neo_clk_ON
            sbbo   gpio0, set_gpio0, 0, 4
.endm

//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// Turn the NEO_CLK off
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.macro		neo_clk_OFF
            sbbo   gpio0, clr_gpio0, 0, 4
.endm
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.macro		gpio_setup
            mov     read_gpio0, GPIO0 | GPIO_DATAOUT
            mov     set_gpio0, GPIO0 | GPIO_SETDATAOUT
            mov     clr_gpio0, GPIO0 | GPIO_CLEARDATAOUT
            zero    &gpio0, 4
            set     gpio0, 26
.endm
